...
CREATE TABLE Service (
    ...
    CONSTRAINT chk_service_type CHECK (service_type IN ('TOURS INDIVIDUALES', 'TOURS GRUPALES', 'TRADUCCIONES'))
);
...
CREATE TABLE Reviews (
    ...
    date DATE NOT NULL,
    description TEXT,
    FOREIGN KEY (from_tourist) REFERENCES Tourist(id) ON DELETE SET NULL,
...
CREATE TABLE UserTrophy (
    ...
    FOREIGN KEY (guide_id) REFERENCES Guide(id) ON DELETE CASCADE,
    FOREIGN KEY (tourist_id) REFERENCES Tourist(id) ON DELETE CASCADE,
    CONSTRAINT chk_only_one_id CHECK ((guide_id IS NOT NULL AND tourist_id IS NULL) OR (guide_id IS NULL AND tourist_id IS NOT NULL))
);
